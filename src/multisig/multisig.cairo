...
mod Account {
    ...
    struct Storage {
        index_to_owner: LegacyMap::<felt, felt>,
        owner_to_index: LegacyMap::<felt, felt>,
        num_owners: felt,
    }

    #[constructor]
    fn constructor(public_keys: Array::<felt>) {
        assert(public_keys.len <= 3, 'public_keys.len <= 3');
        num_owners::write(public_keys.len);
        _set_owners(public_keys)
    }
